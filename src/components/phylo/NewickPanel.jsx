    ...
    <Panel style={{ padding: 16 }}>
      ...
      <textarea readOnly value={newick} rows={5} spellCheck={false} onFocus={(e) => e.target.select()}
        style={{
          ...
          borderRadius: 8, padding: "10px 12px", color: C.textDim, fontFamily: FONT_DISPLAY,
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: 10.5, color: C.textFaint, alignSelf: "center" }}>
      ...
      <textarea value={text} onChange={(e) => setText(e.target.value)} rows={3} spellCheck={false}
        ...
        style={{
          ...
          borderRadius: 8, padding: "10px 12px", color: C.text, fontFamily: FONT_DISPLAY,
          fontSize: 11.5, resize: "vertical",
        }} />
      <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap", alignItems: "center" }}>
        <MiniBtn accent onClick={handleVisualize}><FileInput size={12} /> Visualize pasted tree</MiniBtn>
        {importActive && (
          <>
            <span style={{ fontSize: 11, color: C.qc }}>showing imported tree</span>
            <button onClick={() => { onClearImport(); setText(""); }}
              style={{ all: "unset", cursor: "pointer", fontSize: 11, color: C.textDim, display: "inline-flex", alignItems: "center", gap: 3 }}>
              <X size={11} /> clear
            </button>
          </>
        )}
      ...
      {error && (
        <div style={{ fontSize: 11.5, color: "#f2b3ad", marginTop: 8 }}>{error}</div>
      )}
    </Panel>
  );
}
